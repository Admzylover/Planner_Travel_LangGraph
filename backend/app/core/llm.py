...
class LLMFactory:
    ...
    def _create_llm(
        ...
        **kwargs
        ...
        if provider == "deepseek":
            ...
            if not api_key:
                raise ValueError("MiniMax API Key 未配置，请检查 .env 文件中的 MiniMax_API_KEY")

            print(f"[LLM] MiniMax 配置:")
            print(f"[LLM]   Base URL: {settings.minimax_base_url}")
            print(f"[LLM]   Model: {settings.minimax_model}")
            print(f"[LLM]   API Key: {api_key[:8]}...{api_key[-4:]}")
    ...
    def get_available_providers(cls) -> list:
        ...
        if settings.minimax_api_key:
            providers.append({"name": "minimax", "model": settings.minimax_model})
