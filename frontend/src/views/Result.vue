    ...
    <div v-else class="content">
      ...
      <el-card class="map-card" shadow="hover">
        ...
        <div class="map-legend" v-if="tripPlan?.days?.length">
          <span class="legend-title">路线图例：</span>
          <span
            v-for="(day, index) in tripPlan.days"
            :key="index"
            class="legend-item"
          >
            <span class="legend-color" :style="{ background: getRouteColor(index) }"></span>
            第{{ index + 1 }}天{{ day.city ? `(${day.city})` : '' }}
          </span>
        </div>
      </el-card>
          ...
          >
            <template #title>
              <div class="day-title">
                ...
                <el-tag v-if="day.city" type="success">{{ day.city }}</el-tag>
            ...
            <div class="dining-section">
              ...
              <el-row :gutter="12" v-if="day.meals && day.meals.length > 0">
                <el-col v-for="(meal, mealIndex) in day.meals" :key="mealIndex" :span="8">
                  <el-card shadow="hover" class="meal-card">
                    ...
                    <div class="meal-name">{{ meal.name || '待定' }}</div>
                    <div class="meal-cuisine">{{ meal.description || '当地特色美食' }}</div>
                    <div class="meal-price">预估: ¥{{ meal.estimated_cost || 50 }}</div>
                  </el-card>
                </el-col>
              </el-row>
              <el-empty v-else description="暂无餐饮推荐" :image-size="60" />
            </div>
...
// 路线颜色
const routeColors = ['#FF0000', '#0066FF', '#00CC66', '#FF6600', '#9933FF', '#FF0099', '#00CCCC', '#CC6600']
...
async function initMap() {
  ...
  try {
    ...
    if (!AMap) {
      ...
      // 使用1.4版本，更稳定，不需要安全密钥
      script.src = `https://webapi.amap.com/maps?v=1.4.15&key=${amapKey}&plugin=AMap.Geocoder,AMap.Scale,AMap.ToolBar,AMap.Driving`
      script.onload = () => {
  ...
  geocoder.getLocation(city, (status: string, result: any) => {
    ...
    tripPlan.value.days?.forEach((day: any, dayIndex: number) => {
      ...
      day.attractions?.forEach((attraction: any, attrIndex: number) => {
        ...
        if (attraction.location) {
          ...
          if (lng && lat && lng !== 0 && lat !== 0) {
            ...
            dayPoints.push({ name: attraction.name, position })

            // 创建标记
            const marker = new AMap.Marker({
              position: position,
              title: attraction.name,
              content: `<div style="background:${routeColors[dayIndex % routeColors.length]};color:white;padding:2px 6px;border-radius:50%;font-size:12px;font-weight:bold;">${dayIndex + 1}</div>`
            })
      ...
      if (dayPoints.length >= 2) {
        dayRoutes.push({ dayIndex, points: dayPoints, color: routeColors[dayIndex % routeColors.length] })
      }
    })
    ...
    dayRoutes.forEach((route: any) => {
      console.log(`[Map] 绘制第${route.dayIndex + 1}天路线，共${route.points.length}个点`)
      ...
      // 构建途经点（排除起点和终点）
      const waypoints = route.points.slice(1, -1).map((p: any) => p.position)

      driving.search(route.points[0].position, route.points[route.points.length - 1].position, {
        waypoints: waypoints.length > 0 ? waypoints : undefined
      }, (status: string, result: any) => {
        if (status === 'complete') {
          console.log(`[Map] 第${route.dayIndex + 1}天路线绘制成功`)
        } else {
          console.warn(`[Map] 第${route.dayIndex + 1}天路线绘制失败:`, result)
        }
      })
    })
